...
AND YEAR(PUB_DATE) >= '2020'
ORDER BY PUB_DATE DESC;

#가격이 15,000원 이상 30,000원 이하인 도서 중, 재고가 NULL이 아닌 도서의 제목, 가격, 재고를 조회하시오. 이때 가격 기준 오름차순, 가격이 같다면 재고 기준 내림차순으로 정렬하시오.
...
#제목에 '공부' 또는 **'설계'**가 포함된 도서 중 카테고리가 **'IT'**인 도서의 제목, 카테고리, 가격을 조회하시오. 단, 가격에 10% 부가세를 더한 값을 **'총합계'**라는 별칭으로 계산하여 함께 보여주시오. (힌트: PRICE * 1.1)
SELECT TITLE, CATEGORY, PRICE, PRICE * 1.1 총합계
FROM books
